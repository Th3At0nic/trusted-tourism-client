...
const Checkout = () => {
  ...
  const handleCheckout = (paymentId) => {
    ...
    const orderInfo = {
      ...
      paymentId: paymentId,
      productId: id,
      productName: checkoutData.packageName,
      ...
      <div className="container checkout-area">
        {!success && (
          <h2 className="text-theme">
            <FontAwesomeIcon icon={["fas", "shopping-cart"]} /> Checkout
          </h2>
        )}
        {success && (
          <div className="text-center">
            <h2 className="text-primary ">
              <FontAwesomeIcon icon={["fas", "check"]} /> Checkout Successful
            </h2>
            <Link className="btn btn-info " to="/dashboard/orderList">
              <FontAwesomeIcon icon={["fas", "list"]} /> Check Orders
            </Link>
          </div>
        )}

        <div className="row" id="checkoutWrapper">
          <div className="col-lg-8 col-md-8 col-sm-8 col-xs-12 checkout-left">
            <ShowCheckoutProduct service={checkoutData}></ShowCheckoutProduct>
          </div>

          <div className="col-lg-4 col-md-4 col-sm-4 col-xs-12 checkout-right bg-white p-2 pt-3 pb-3 rounded">
            <h2 className="text-theme pb-3">
              <FontAwesomeIcon icon={["fas", "credit-card"]} /> Payment
              Information
            </h2>
